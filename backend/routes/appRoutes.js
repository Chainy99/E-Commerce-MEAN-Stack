...
function secondtimecart(req, res, oldcart, newmedicine, userid) {
    var oldavail = false;
   
    var newtotal = oldcart.total + newmedicine.medicineprice;
    var tot;
   
    var oldmedicinejsonarr = oldcart['medicine']
   
    for (var i = 0; i < oldmedicinejsonarr.length; i++) {
        if (oldmedicinejsonarr[i]._id == newmedicine._id) {
            oldavail = true;
        }
    }
    ...
    if (oldavail) {
        ...
        for (var i = 0; i < oldmedicinejsonarr.length; i++) {
            if (oldmedicinejsonarr[i]._id == newmedicine._id) {
                oldmedicinejsonarr[i].qty += 1;
                oldcart.total += oldmedicinejsonarr[i].medicineprice
                tot = oldcart.total
                console.log(oldcart.total);
            }
        }

        console.log(oldmedicinejsonarr);

        Cart.updateOne({ _id: oldcart._id }, {
            medicine: oldmedicinejsonarr,
            total: tot
        }, function (err, ct) {
            ...
            if (err) {
                ...
                res.json({ msg: "medicine added to the cart" })
            }
        })
    }
    else {
        ...
        oldmedicinejsonarr.push(newmedicine)
        Cart.updateOne({ _id: oldcart._id }, {
            medicine: oldmedicinejsonarr,
            total: newtotal
        }, function (err, ct) {
...
router.get('/getcartitem', verifyToken, (req, res, next) => {
    Cart.find({ whichuser: req.userId }, (err, medicines) => {
        if (err) {
...
function deletefromcart(req, res, oldcart, newmedicine, userid) {
    var temp = [];
    var total;
    var oldmedicinejsonarr = oldcart['medicine']
    ...
    for (var i = 0; i < oldmedicinejsonarr.length; i++) {
        if (oldmedicinejsonarr[i]._id != newmedicine._id) {
            temp.push(oldmedicinejsonarr[i])
        }
        else {
            oldcart.total -= newmedicine.medicineprice * newmedicine.qty;
            total = oldcart.total;
        }
    }
    if (total == 0) {
        ...
        res.json({ msg: "medicine deleted from the cart" })
